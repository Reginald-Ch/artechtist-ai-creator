...
import { Brain, Bot, MessageSquare, Play, Save, Mic, ArrowLeft, Plus, Undo, Redo, ChevronDown, Menu, Info, Zap, Layout, X, Send, RotateCcw, MicIcon, StopCircle, Speaker, Sparkles, Smartphone, Settings, ArrowRight, RefreshCw } from "lucide-react";
...
import { detectSpatialRelationships, getNearbyNodes, type ProximityInfo } from "@/utils/spatialConnectionDetector";
...
const SimplifiedBotBuilder = ({ template }: SimplifiedBotBuilderProps) => {
  ...
  // PHASE 1-5: Spatial auto-connection state
  const [autoConnectEnabled, setAutoConnectEnabled] = useState(true);
  const [autoConnectMode, setAutoConnectMode] = useState<'horizontal' | 'vertical' | 'mixed'>('horizontal');
  const [detectionRadius, setDetectionRadius] = useState(250);
  const [hoveredNode, setHoveredNode] = useState<string | null>(null);
  const [proximityIndicators, setProximityIndicators] = useState<ProximityInfo[]>([]);
  ...
  // Apply template data when component mounts (PHASE 5: Disable auto-connect during template load)
  useEffect(() => {
    if (template) {
      ...
      // Re-enable auto-connect after template loads
  ...
  const onConnect = useCallback((params: Connection) => {
    ...
    // PHASE 5: If manual connection, remove conflicting auto-generated edges
    setEdges((eds) => {
      const autoEdgeToRemove = eds.find(
        e => e.source === params.source && 
             e.target === params.target && 
             e.data?.autoGenerated
      );
      
      let cleanedEdges = eds;
      if (autoEdgeToRemove) {
        cleanedEdges = eds.filter(e => e.id !== autoEdgeToRemove.id);
      }
  ...
  // PHASE 2: Auto-connect on node drag stop
  const onNodeDragStop = useCallback((event: React.MouseEvent, node: Node) => {
    if (!autoConnectEnabled || isLoadingTemplate) return;
    
    // Save state before auto-connections
    undoRedo.saveState(nodes, edges);
    
    // Detect spatial relationships
    const suggestions = detectSpatialRelationships(nodes, edges, {
      detectionRadius,
    ...
    if (relevantSuggestions.length > 0) {
      // Create new edges automatically
      const newEdges = relevantSuggestions.map(s => ({
        id: `auto-${s.source}-${s.target}`,
        source: s.source,
        target: s.target,
        markerEnd: { type: MarkerType.ArrowClosed },
        animated: true,
        style: { 
          stroke: 'hsl(var(--primary))',
          strokeWidth: 3,
          strokeDasharray: '5,5', // Dashed to show auto-generated
        },
      ...
      // Show feedback toast
      toast({
        title: t('toast.autoConnected') || "Auto-connected",
        description: `Created ${newEdges.length} connection(s) based on proximity`,
      });
    }
  }, [autoConnectEnabled, isLoadingTemplate, nodes, edges, autoConnectMode, detectionRadius, undoRedo, setEdges, t]);

  // PHASE 3: Visual feedback on node hover
  const onNodeMouseEnter = useCallback((event: React.MouseEvent, node: Node) => {
    if (!autoConnectEnabled || isLoadingTemplate) return;
    
    const nearby = getNearbyNodes(node, nodes, detectionRadius);
    
    setHoveredNode(node.id);
    setProximityIndicators(nearby);
  }, [nodes, autoConnectEnabled, isLoadingTemplate, detectionRadius]);

  const onNodeMouseLeave = useCallback(() => {
    setHoveredNode(null);
    setProximityIndicators([]);
  }, []);
  ...
  // PHASE 5: Edge management utilities
  const removeAutoGeneratedEdges = useCallback(() => {
    setEdges(eds => eds.filter(e => !e.data?.autoGenerated));
    toast({
      title: "Auto-connections cleared",
      description: "Removed all auto-generated connections",
    });
  }, [setEdges]);

  const refreshAutoConnections = useCallback(() => {
    // Remove existing auto-generated edges
    setEdges(eds => eds.filter(e => !e.data?.autoGenerated));
    
    // Detect new connections
    const suggestions = detectSpatialRelationships(nodes, edges.filter(e => !e.data?.autoGenerated), {
      detectionRadius,
      mode: autoConnectMode,
      allowCycles: false,
    });
    ...
    if (suggestions.length > 0) {
      ...
      toast({
        title: "Auto-connections refreshed",
        description: `Created ${newEdges.length} new connection(s)`,
      });
    } else {
      toast({
        title: "No connections found",
        description: "No nodes are close enough to auto-connect",
      });
    }
  }, [nodes, edges, detectionRadius, autoConnectMode, setEdges]);
  ...
  // Additional keyboard shortcuts for auto-connect
  useEffect(() => {
        ...
        switch (e.key.toLowerCase()) {
          case 'a':
            ...
            setAutoConnectEnabled(prev => {
              const newVal = !prev;
              toast({
                title: newVal ? "Auto-connect enabled" : "Auto-connect disabled",
              });
              return newVal;
            });
            break;
          case 'h':
            e.preventDefault();
            setAutoConnectMode('horizontal');
            toast({ title: "Horizontal mode" });
            break;
          case 'v':
            e.preventDefault();
            setAutoConnectMode('vertical');
            toast({ title: "Vertical mode" });
            break;
          case 'r':
            e.preventDefault();
            refreshAutoConnections();
            break;
          case 'c':
            e.preventDefault();
            removeAutoGeneratedEdges();
            break;
        }
      }
    };
  ...
  }, [refreshAutoConnections, removeAutoGeneratedEdges]);
        ...
        <div className="flex flex-col lg:flex-row flex-1 gap-0 border-t border-border">
          ...
          <div className="flex-[2] lg:border-l lg:border-r border-border bg-background order-2 lg:order-1">
            <div className="h-12 px-6 border-b border-border flex items-center justify-between bg-gradient-to-r from-primary/5 to-secondary/5">
              ...
              {/* PHASE 4: Auto-Connect Settings Panel */}
              <div className="flex items-center gap-3 border-l pl-4">
                <TooltipProvider>
                  <div className="flex items-center gap-2">
                    <Label className="text-xs font-medium whitespace-nowrap">Auto-Connect:</Label>
                    <Tooltip>
                      ...
                      <TooltipContent>
                        <p>Automatically connect nodes based on proximity (Alt+A)</p>
                      </TooltipContent>
                    </Tooltip>
                  </div>

                  <Select value={autoConnectMode} onValueChange={(value: any) => setAutoConnectMode(value)}>
                  ...
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Button 
                        variant="ghost" 
                        size="sm"
                        onClick={refreshAutoConnections}
                    ...
                    <TooltipContent>
                      <p>Refresh auto-connections (Alt+R)</p>
                    </TooltipContent>
                  </Tooltip>
                  ...
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Button 
                        variant="ghost" 
                        size="sm"
                        onClick={removeAutoGeneratedEdges}
                        className="h-8 w-8 p-0"
                      >
                    ...
                    <TooltipContent>
                      <p>Clear auto-connections (Alt+C)</p>
                    </TooltipContent>
                  </Tooltip>
                    ...
                    <TooltipTrigger asChild>
                      <Dialog>
                        ...
                        <DialogContent className="sm:max-w-md">
                          <DialogHeader>
                            <DialogTitle>Auto-Connect Settings</DialogTitle>
                          </DialogHeader>
                          <div className="space-y-4 py-4">
                            <div className="space-y-2">
                              <Label>Detection Radius: {detectionRadius}px</Label>
                              <Slider
                                value={[detectionRadius]}
                                onValueChange={([v]) => setDetectionRadius(v)}
                                min={100}
                                max={500}
                                step={50}
                                className="w-full"
                              />
                              <p className="text-xs text-muted-foreground">
                                How close nodes must be to auto-connect
                              </p>
                            </div>
                            ...
                            <div className="space-y-2">
                              ...
                              <div className="text-xs space-y-1 text-muted-foreground">
                                <p><kbd className="px-1 py-0.5 rounded bg-muted">Alt+A</kbd> Toggle auto-connect</p>
                                <p><kbd className="px-1 py-0.5 rounded bg-muted">Alt+H</kbd> Horizontal mode</p>
                                <p><kbd className="px-1 py-0.5 rounded bg-muted">Alt+V</kbd> Vertical mode</p>
                                <p><kbd className="px-1 py-0.5 rounded bg-muted">Alt+R</kbd> Refresh connections</p>
                                <p><kbd className="px-1 py-0.5 rounded bg-muted">Alt+C</kbd> Clear auto-connections</p>
                              </div>
                            </div>
                          </div>
                        </DialogContent>
            ...
            <div className="h-[calc(100vh-8rem)] relative">
              ...
              >
                ...
                {/* PHASE 3: Proximity indicators overlay */}
                {proximityIndicators.length > 0 && hoveredNode && (
                  <div className="absolute inset-0 pointer-events-none z-10">
                    {proximityIndicators.slice(0, 3).map((indicator, idx) => {
                      const hoveredNodeData = nodes.find(n => n.id === hoveredNode);
                      const targetNodeData = nodes.find(n => n.id === indicator.nodeId);
                      
                      if (!hoveredNodeData || !targetNodeData) return null;
                      
                      const startX = hoveredNodeData.position.x + 140;
                      const startY = hoveredNodeData.position.y + 100;
                      const endX = targetNodeData.position.x + 140;
                      const endY = targetNodeData.position.y + 100;
                      const midX = (startX + endX) / 2;
                      const midY = (startY + endY) / 2;
                      
                      return (
                        <div key={`indicator-${indicator.nodeId}`}>
                          ...
                          >
                            <div className="relative flex items-center justify-center">
                              <div className="w-10 h-10 rounded-full bg-primary/20 animate-ping absolute"></div>
                              <div className="w-8 h-8 rounded-full bg-primary/30 backdrop-blur-sm flex items-center justify-center border-2 border-primary/50">
                                <ArrowRight 
